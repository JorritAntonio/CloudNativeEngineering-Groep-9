...
export class CommentRepository {
    ...
    toComment = async(commentDocument: CosmosCommentDocument): Promise<Comment> => {
        ...
        const user = await userRepository.findUserById(parseInt(commentDocument.user));
        const thread = await threadRepository.findThreadById(commentDocument.thread);

        if (!user) {
            throw new Error("Cannot change commentDocument to Comment due to missing user");
        }

        if (!thread) {
            throw new Error("Cannot change commentDocument to Comment due to missing thread");
        }
