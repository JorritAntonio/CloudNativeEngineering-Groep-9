...
export class ThreadRepository {
    ...
    toThread = async(threadDocument: CosmosThreadDocument): Promise<Thread> => {
        ...
        const comments = await Promise.all(threadDocument.comments.map(commentId => commentRepository.findCommentById(commentId)));

        if (!user) {
            throw new Error("Cannot change threadDocument to Thread due to missing user");
        }
